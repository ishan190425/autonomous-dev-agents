...
describe('template bundling — #150 regression tests', () => {
  describe('template file existence', () => {
    ...
    /**
     * Validates templates are bundled in CLI package for npm distribution.
     * Fix implemented in C684.
     */
    it('templates exist in CLI package (npm bundled setup)', () => {
      expect(
  ...
  describe('package.json configuration', () => {
    ...
    /**
     * Validates package.json includes templates for npm bundling.
     * Fix implemented in C684.
     */
    it('package.json files array includes templates', () => {
      const packageJsonPath = path.join(CLI_ROOT, 'package.json');
      const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf-8'));
...
describe('npm pack simulation', () => {
  /**
   * Validates templates are included in npm package.
   * Fix implemented in C684.
   */
  it('files array includes both dist and templates', () => {
    const packageJsonPath = path.join(CLI_ROOT, 'package.json');
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf-8'));

    const currentFiles = packageJson.files || [];

    // After C684 fix: both dist and templates are bundled
