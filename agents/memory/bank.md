...
- **Last:** Cognitive Memory Implementation Plan (C816) — `docs/frontier/cognitive-memory-implementation-plan-c816.md`. Comprehensive implementation spec bridging #113 (research) with #180 (SQLite). SQLite schema (4 tables: memory_entries, memory_embeddings, memory_references, heat_history). Heat scoring algorithm (recency decay + reference boost). Tier thresholds (Hot ≥0.75/100 items, Warm 0.25-0.75/10K, Cold <0.25/unlimited). Innate memory protection (RULES.md, roster, playbooks). CognitiveMemoryService TypeScript API. CLI commands (search/heat/maintain). 2-phase migration path. Sprint 3 Week 1-2 roadmap. Commented #113, #180.
- **Next:** Prototype SQLite + sqlite-vec integration. Local embedding pipeline (all-MiniLM-L6-v2).
